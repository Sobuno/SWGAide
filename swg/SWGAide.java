...
public final class SWGAide {
    ...
    /**
     * Helper method which determines if SWGAide is allowed to create and delete
     * a file outside of user space. If there is an error this method writes the
     * error to SWGAide's log file and displays a dialog with the options to
     * exit SWGAide or continue at the user's peril.
     */
    private static void checkWinAdminMode() {
        try {
            File f = new File("C:\\swgaide_test.txt");
            f.createNewFile();
            f.delete();
            return;
        } catch (Throwable e) {
            ZString msg = ZString.fz("Error testing administrator mode:%n" +
                    "%s%nOS: %s%nSee Windows section in SWGAide_README.txt%n" +
